...
#define TracyGpuContext( device, devicectx ) tracy::s_dx11Ctx.ptr = (tracy::Dx11Ctx*)tracy::tracy_malloc( sizeof( tracy::Dx11Ctx ) ); new(tracy::s_dx11Ctx.ptr) tracy::Dx11Ctx( device, devicectx );
#define TracyGpuNamedZone( varname, devicectx, name ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,__LINE__) { name, __FUNCTION__,  __FILE__, (uint32_t)__LINE__, 0 }; tracy::Dx11CtxScope varname( &TracyConcat(__tracy_gpu_source_location,__LINE__), devicectx );
#define TracyGpuNamedZoneC( varname, devicectx, name, color ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,__LINE__) { name, __FUNCTION__,  __FILE__, (uint32_t)__LINE__, color }; tracy::Dx11CtxScope varname( &TracyConcat(__tracy_gpu_source_location,__LINE__), devicectx );
#define TracyGpuZone( devicectx, name ) TracyGpuNamedZone( ___tracy_gpu_zone, devicectx, name )
#define TracyGpuZoneC( devicectx, name, color ) TracyGpuNamedZoneC( ___tracy_gpu_zone, devicectx, name, color )
#define TracyGpuCollect( devicectx ) tracy::s_dx11Ctx.ptr->Collect( devicectx );

#ifdef TRACY_HAS_CALLSTACK
#  define TracyGpuNamedZoneS( varname, devicectx, name, depth ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,__LINE__) { name, __FUNCTION__,  __FILE__, (uint32_t)__LINE__, 0 }; tracy::Dx11CtxScope varname( &TracyConcat(__tracy_gpu_source_location,__LINE__), devicectx, depth );
#  define TracyGpuNamedZoneCS( varname, devicectx, name, color, depth ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,__LINE__) { name, __FUNCTION__,  __FILE__, (uint32_t)__LINE__, color }; tracy::Dx11CtxScope varname( &TracyConcat(__tracy_gpu_source_location,__LINE__), devicectx, depth );
#  define TracyGpuZoneS( devicectx, name, depth ) TracyGpuNamedZoneS( ___tracy_gpu_zone, devicectx, name, depth )
#  define TracyGpuZoneCS( devicectx, name, color, depth ) TracyGpuNamedZoneCS( ___tracy_gpu_zone, devicectx, name, color, depth )
#else
#  define TracyGpuNamedZoneS( varname, devicectx, name, depth ) TracyGpuNamedZone( varname, devicectx, name )
#  define TracyGpuNamedZoneCS( varname, devicectx, name, color, depth ) TracyGpuNamedZoneC( varname, devicectx, name, color )
#  define TracyGpuZoneS( devicectx, name, depth ) TracyGpuZone( devicectx, name )
#  define TracyGpuZoneCS( devicectx, name, color, depth ) TracyGpuZoneC( name, color )
#endif
